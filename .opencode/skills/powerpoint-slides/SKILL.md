...
description: Create visually rich, editable PowerPoint (.pptx) presentations with PptxGenJS, native OMML math, and Graphviz/Mermaid/TikZ diagrams. Use ONLY when the user wants PPT/PPTX output or slide work: pptx, powerpoint, PPT, make a ppt, presentation, slides, deck, pitch deck, 做PPT, 做幻灯片, 做个报告, convert paper to slides (when PPT is implied). Do NOT trigger for beamer/latex/.tex/tikz work.
license: MIT
metadata:
  source: https://github.com/Noi1r/powerpoint-skill
  upstream-argument-hint: "[action] [file] — actions: create, compile, review, audit, pedagogy, excellence, visual-check, validate, extract-figures"
---

# PowerPoint Slides Workflow

> **opencode paths.** Scripts in this skill live next to this file, not under
> `~/.claude/skills/`. Resolve them relative to this skill directory, e.g.
> `.opencode/skills/powerpoint-slides/scripts/render_latex.py`. Formulas and
> diagrams are optional — a deck only needs PptxGenJS unless math/diagrams are
> requested. LibreOffice (`soffice`), Poppler (`pdftoppm`), Graphviz, and
> Mermaid are optional; if a tool is missing, degrade gracefully and report it
